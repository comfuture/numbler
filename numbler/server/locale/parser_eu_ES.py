...
monthdict = {'urria': 10, 'abendua': 12, 'ekaina': 6, 'api': 4, 'uzt': 7, 'urtarrila': 1, 'abe': 12, 'uztaila': 7, 'abu': 8, 'ira': 9, 'mar': 3, 'apirila': 4, 'iraila': 9, 'azaroa': 11, 'mai': 5, 'ots': 2, 'abuztua': 8, 'maiatza': 5, 'otsaila': 2, 'aza': 11, 'eka': 6, 'martxoa': 3, 'urt': 1, 'urr': 10}
t_SHORTMONTH = r'(?i)urt|ots|mar|api|mai|eka|uzt|abu|ira|urr|aza|abe'
t_LONGMONTH = r'(?i)urtarrila|otsaila|martxoa|apirila|maiatza|ekaina|uztaila|abuztua|iraila|urria|azaroa|abendua'
...
t_DAYINWEEK = r'ig|al|as|az|og|or|lr|igandea|astelehena|asteartea|asteazkena|osteguna|ostirala|larunbata'
...
def p_shortdate(p):
    ' shortdate : INTEGER CUSTOMLIT0 INTEGER CUSTOMLIT0 INTEGER '
    monthasint = True
    month = p[3]
    day = int(p[5])
    year = int(p[1])
...
def p_meddate(p):
    ' meddate : INTEGER CUSTOMLIT0 month CUSTOMLIT0 INTEGER '
    monthasint = False
    day = int(p[5])
    month = p[3]
    year = int(p[1])
...
def p_longdate(p):
    ' longdate : DAYINWEEK CUSTOMLIT1 datesep INTEGER CUSTOMLIT2 datesep month CUSTOMLIT3 datesep INTEGER CUSTOMLIT4 '
    monthasint = False
    day = int(p[10])
    month = p[7]
    year = int(p[4])
