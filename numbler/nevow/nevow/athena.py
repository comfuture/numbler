    ...
    def render_liveFragment(self, ctx, data):
        ...
        return (
            ...
            ctx.tag(**{'xmlns:athena': ATHENA_XMLNS_URI,
		       'id':'athenaid' + str(self._athenaID),
...
def expose(*funcObjs,**kwargs):
    ...
    if 'inherit' is a kw argument then the parent allowedmethods will be added
    ...
    def exposedMethodAdvice(cls):
        ...
        if 'inherit' in kwargs:
            if len(cls.__mro__) > 1 and 'allowedMethods' in cls.__mro__[1].__dict__:
                cls.allowedMethods.extend(cls.__mro__[1].allowedMethods)
        return cls
