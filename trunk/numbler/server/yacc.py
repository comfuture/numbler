#
# NOTE: This version of ply is slightly hacked (I can't remember why at this point)
# 
...
def lr_read_tables(module=tab_module,tabmoduleparent=None,optimize=0):
    global _lr_action, _lr_goto, _lr_productions, _lr_method
    try:
        if tabmoduleparent:
            execstr = "from %s import %s as parsetab" % (tabmoduleparent,module)
        else:
            execstr = "import %s as parsetab" % module
        exec execstr
...
def yacc(method=default_lr, debug=yaccdebug, module=None, tabmodule=tab_module, start=None, check_recursion=1, optimize=0,write_tables=1,debugfile=debug_file,outputdir='',tabmoduleparent=None):
    global yaccdebug
    yaccdebug = debug
    ...
    if (optimize and lr_read_tables(tabmodule,tabmoduleparent,1)):
        # Read parse table
        del Productions[:]
        ...
        if not lr_read_tables(tabmodule,tabmoduleparent):
